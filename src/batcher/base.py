...
class EEGDataset(Dataset):
    def __init__(self, filenames, sample_keys=None, chunk_len=500, num_chunks=10, ovlp=50, root_path="", population_mean=0, population_std=1, gpt_only=False, normalization=True, start_samp_pnt=-1):
        if root_path == "":
    ...
    def __getitem__(self, idx):
        try:
            ...
            print(f"Error processing file {self.filenames[idx]}: {e}")
            data = {"inputs": torch.zeros((self.num_chunks, 500)), "attention_mask": torch.zeros(self.num_chunks)}
        return data
